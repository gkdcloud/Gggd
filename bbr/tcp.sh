...
startbbrmod(){
	...
	if [[ "${release}" == "centos" ]]; then
		...
		wget -N --no-check-certificate https://ghproxy.com/http://${github}/bbr/tcp_tsunami.c
...
startbbrmod_nanqinlang(){
	...
	if [[ "${release}" == "centos" ]]; then
		yum install -y make gcc
		mkdir bbrmod && cd bbrmod
		wget -N --no-check-certificate https://ghproxy.com/https://raw.githubusercontent.com/chiakge/Linux-NetSpeed/master/bbr/centos/tcp_nanqinlang.c
